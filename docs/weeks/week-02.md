...
- [ ] 絶対誤差と相対誤差を計算できる
- [ ] 補正（Correction）と校正（Calibration）の違いを説明できる
...
### 7.2 校正（Calibration）/ キャリブレーション

::: info 定義
**Calibration**：計測器の校正を行い、基準となる値との対応関係を調べることです。
:::
...
::: info 実務的な考え方
校正点が多いほど、センサの入出力関係をより適切に表しやすくなりますが、時間や手間も増えます。用途に応じて、2点校正・多点校正を使い分けます。
:::
...
FlexiForce のような薄膜力センサは、使用範囲によっては**完全な線形にならず、非線形性**を示すことがあります。
そのため、2点だけで直線を決めるのではなく、**多点で校正して校正曲線を求める**ことが重要です。
...
::: tip 💡 ポイント
- 力センサの校正では、出力がきれいな直線になるとは限りません。
- 広い測定範囲では、非線形な校正曲線として扱うほうが適切なことがあります。
- ただし、狭い範囲だけを使う場合は、線形近似で十分なこともあります。
:::
...
::: warning 注意
計測値を書くときは、「桁を多く書けば正確になる」のではなく、**測定の不確かさに見合った桁数で表す**ことが大切です。
:::
...
講義内容の理解を定着させるため、以下の課題に取り組みましょう。

### 課題1：誤差の分類
次の例を、人為的誤差・系統誤差・不定誤差（偶然誤差）のいずれに分類するか答え、理由を1行ずつ書きなさい。

1. 目盛りを読み違えた
2. センサが毎回 +0.3 V 高く出る
3. 電源ノイズで値が毎回少しずつ変動する
4. 記録で 2.31 を 3.21 と書いてしまった

### 課題2：誤差・絶対誤差・相対誤差
真値が 50.0 mm、計測値が 49.6 mm だったとする。
...
1. この 5 個の値は何と呼ばれるか
2. このデータから代表値を1つ考えるとしたら、どのあたりの値が妥当か
3. その理由を説明しなさい
...
さらに、**12.3 と 12.30 はなぜ同じではないのか** を1〜2文で説明しなさい。

### 課題6：校正の考え方
温度センサが、0℃ で 0.20 V、100℃ で 4.20 V を示した。
...
狙いは、今日学んだ「誤差」「確度と精度」「補正と校正」「統計的な見方」を、定義の暗記だけで終わらず、実際に判断・計算できるようにすることです。
