...
|---|---|---|---|
| ポテンショメータ | 回転角・位置 | 角度が変わる → 摺動子の位置が変わる → 分圧比が変わる | 電圧 |
| 光学式エンコーダ | 回転角 | 円板が回る → スリットが光を通す/遮る | パルス |
| インクリメンタル型エンコーダ | 相対角度・回転方向 | 回転する → A相・B相パルスが発生する → パルス数と位相差を読む | パルス数・位相差 |
| Z相 | 原点位置 | 1回転に1回だけ基準スリットを読む | 原点パルス |
| アブソリュート型エンコーダ | 絶対角度 | 角度位置ごとに異なるパターンを読む | ビット列 |
| 機械接点式エンコーダ | 回転位置 | 接点のON/OFF組合せが変わる | ON/OFF信号 |
| タコジェネレータ | 回転速度 | 回転が速くなる → 発電電圧が大きくなる | 電圧 |
